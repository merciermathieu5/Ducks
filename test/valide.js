...
(async () => {
  ...
  egal(S.SECOURS_ROSTER.length, 25, '25 joueurs dans la formation de secours (pro, aucun backup)');
  const dub = S.SECOURS_ROSTER.find(j => j.nom === 'Pierre-Luc Dubois');
  egal(dub.salaire, 6000000, 'Salaire Dubois');
  egal(dub.ct, 1, 'Contrat Dubois 1 an');
  egal(S.SECOURS_Y21.length, 23, '23 patineurs dans la référence Y21 (XtraStats, validés par ratios)');
  const dubY21 = S.SECOURS_Y21.find(x => x.nom === 'Pierre-Luc Dubois');
  egal(dubY21.pts, 68, 'Dubois 68 points en Y21');
  const tuchY21 = S.SECOURS_Y21.find(x => x.nom === 'Alex Tuch');
  egal(tuchY21.hits, 243, 'Tuch 243 MEÉ en Y21');
  const guY21 = S.SECOURS_Y21.find(x => x.nom === 'Denis Gurianov');
  egal(guY21.gp, 21, 'Gurianov : 21 matchs sous les couleurs du club (échange en cours de Y21, ligne transcrite telle quelle)');
  const orY21 = S.SECOURS_Y21.find(x => x.nom === 'Ryan O\'Rourke');
  egal(orY21?.pts, 19, 'O\'Rourke 19 points en Y21 (nom avec apostrophe géré)');
  ...
  egal(profilDe('Denis Gurianov').profil, 'Elite', 'Gurianov → Elite');
  tableauEgal(profilDe('Denis Gurianov').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Semyon Arguchintsev').profil, 'Playmaker', 'Arguchintsev → Playmaker');
  tableauEgal(profilDe('Semyon Arguchintsev').stats, ['assists','pts'], 'Stats Playmaker = A, P');
  egal(profilDe('Pierre-Luc Dubois').profil, 'Two-Way Forward', 'Dubois → Two-Way Forward');
  tableauEgal(profilDe('Pierre-Luc Dubois').stats, ['pts','pmrang'], 'Stats Two-Way = P, +/-');
  egal(profilDe('Alex Tuch').profil, 'Power Forward', 'Tuch → Power Forward (99 de robustesse!)');
  egal(profilDe('Tom Wilson').profil, 'Power Forward', 'Wilson → Power Forward');
  egal(profilDe('Oliver Wahlstrom').profil, 'Sniper', 'Wahlstrom → Sniper');
  egal(profilDe('Joni Ikonen').profil, 'Sniper', 'Ikonen → Sniper');
  egal(profilDe('Eric Florchuk').profil, 'Depth Forward', 'Florchuk → attaquant de profondeur');
  egal(profilDe('Peyton Krebs').profil, 'Prospect Elite', 'Krebs (24 ans) → Prospect Elite');
  egal(profilDe('Peyton Krebs').mat, 'ELITE', 'Prospect Elite → même matrice ELITE');
  egal(profilDe('Ethan Phillips').profil, 'Prospect Grinder', 'Phillips (24 ans) → Prospect Grinder');
  egal(profilDe('Justin Sourdif').profil, 'Prospect Two-Way Forward', 'Sourdif (23 ans) → Prospect Two-Way');
  egal(profilDe('Ryan Pulock').profil, 'DEliteShutdown', 'Pulock → DEliteShutdown');
  egal(profilDe('Connor Hall').profil, 'DDefensive', 'Hall → DDefensive');
  egal(profilDe('Philippe Myers').profil, 'DDefensive', 'Myers (99 de robustesse) → DDefensive');
  egal(profilDe('Ryan O\'Rourke').profil, 'Prospect DEliteShutdown', 'O\'Rourke (23 ans) → Prospect DEliteShutdown');
  egal(profilDe('Ryker Evans').profil, 'Prospect DEliteQB', 'Evans (24 ans) → Prospect DEliteQB');
  egal(profilDe('Ethan Edwards').profil, 'Prospect DOffensive', 'Edwards (23 ans) → Prospect DOffensive');
  egal(profilDe('Michael McNiven').profil, 'Starter Goalie', 'McNiven (OV 83) → Starter Goalie');
  tableauEgal(profilDe('Michael McNiven').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Cole Brady').profil, 'Backup Goalie', 'Brady (OV 79, 24 ans) → Backup Goalie (règle 6.2.5, peu importe l\'âge)');
  tableauEgal(profilDe('Cole Brady').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const jk = joueurDe('Denis Gurianov');
  ...
  const perso = {ELITE: {pts: {82: [120,110,100,90,50,-1]}}};
  ...
  const fixtureXtra = [
    ...
    '    [Pierre-Luc Dubois](https://x/#Pierre-Luc Dubois) ANAHEIM         C   82  30  45  75 200  30 1700 100 2.44',
    '    [Jonathan Drouin](https://x/#Jonathan Drouin) CAROLINA        C   82  27  61  88 235  56 2144 129 2.87',
    '    [* Backup_RW](https://x/#Backup_RW) ANAHEIM         LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'ANAHEIM');
  egal(x.length, 2, 'Deux patineurs ANAHEIM extraits (Caroline exclue)');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('ANAHEIM'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 78232000, 'Masse salariale des 25 pros = 78 232 000 $');
  ok(!doc.querySelector('#alignSommaire .stat-carte').classList.contains('alerte'), 'Sous le plafond : aucune alerte');

  console.log('— Divers');
  egal(S.matchsEquipe(), 0, 'Fiche ANAHEIM 0-0-0 → 0 match d\'équipe');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Pierre-Luc Dubois', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jDub = S.SECOURS_ROSTER.find(x2=>x2.nom==='Pierre-Luc Dubois');
    const prodRepli = S.productionDe(jDub);
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Pierre-Luc Dubois'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 68, 'Points Y21 de Dubois');
  const carteDub = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Pierre-Luc Dubois');
  ok(!!carteDub, 'Carte de Dubois rendue en mode Y21');
  ok(carteDub.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jDu = S.ETAT.roster.find(x2=>x2.nom==='Pierre-Luc Dubois');
  const evPtsD = S.evaluerStat('pts', S.valeurStat(jDu, prodY21, 'pts', {}), S.seuilsPour(jDu, 'pts', {}, {}).seuils);
  egal(evPtsD.statut, 'excellente',
    'Dubois Excellente en points : 68 = seuil Mémorable exactement, or le tableau 18 exige un dépassement STRICT');
  const jKr = S.ETAT.roster.find(x2=>x2.nom==='Peyton Krebs');
  const evPtsK = S.evaluerStat('pts', S.valeurStat(jKr, S.productionDe(jKr), 'pts', {}), S.seuilsPour(jKr, 'pts', {}, {}).seuils);
  egal(evPtsK.statut, 'correcte', 'Krebs Correcte en points (53 < 54 ≤ 62, Prospect Elite OV 80) — à un point de Satisfaisante');
  ok(!carteDub.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
